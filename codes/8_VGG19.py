...
for cls in BINARY_CLASSES:
    ...
    for img in glob.glob(os.path.join(TRAIN_PATH, cls, '*.' + IMG_FORMAT)):
        counter += 1
        X_train.append(tensorflow.keras.applications.vgg19.preprocess_input(plt.imread(img)))
...
for cls in BINARY_CLASSES:
    ...
    for img in glob.glob(os.path.join(VAL_PATH, cls, '*.' + IMG_FORMAT)):
        counter += 1
        X_val.append(tensorflow.keras.applications.vgg19.preprocess_input(plt.imread(img)))
...
# #### 4.3 Transfer Learning VGG19

# In[6]:


# Load VGG19
vgg = tensorflow.keras.applications.vgg19.VGG19()

...
for cls in BINARY_CLASSES:
    ...
    for img in glob.glob(os.path.join(TRAIN_PATH, cls, '*.' + IMG_FORMAT)):
        counter += 1
        X_train.append(tensorflow.keras.applications.resnet50.preprocess_input(plt.imread(img)))
...
for cls in BINARY_CLASSES:
    ...
    for img in glob.glob(os.path.join(VAL_PATH, cls, '*.' + IMG_FORMAT)):
        counter += 1
        X_val.append(tensorflow.keras.applications.resnet50.preprocess_input(plt.imread(img)))
...
# #### 4.3 Transfer Learning ResNetV2
